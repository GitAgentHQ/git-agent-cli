...
func TestInitCmd_ConfigExists(t *testing.T) {
	dir := t.TempDir()
	...
	err := cmd.ExecuteArgs([]string{"init", "--config", ymlPath})
	requireInitRegistered(t, err)
	if err == nil {
		t.Fatal("expected error when project.yml exists without --force, got nil")
	}
}
