...
- **`graph`** (parent): every command that reads or audits the agent Event Log or its derived indexes (co-change, AST). Children: `status`, `verify`, `rebuild`, `sync`, `impact`, `timeline`, `diagnose`, `provenance`, `callers`, `callees`, `node`, `query`, `affected`. **No graph read/audit command lives at the top level.** A new forensic/query command over the graph goes under `graph`.
