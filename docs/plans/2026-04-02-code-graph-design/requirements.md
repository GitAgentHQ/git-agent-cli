...
| T16 | **Capture diff accumulation** -- raw `git diff` includes changes from prior uncaptured tool calls, causing incorrect file attribution in later actions | High | High | Mitigated: delta-based capture via `capture_baseline` table. Each capture computes `git hash-object` per changed file and only attributes files whose hash differs from the stored baseline. Baseline updated after every successful capture. |
| T17 | **File rename breaks co-change continuity** -- renamed files lose their co-change history because `files.path` is the primary key | Medium | Medium | Mitigated: `renames` table tracks `old_path -> new_path` per commit. Blast-radius queries union results from all historical paths via the renames table. |
| T18 | **History rewrite invalidates index state** -- force-push or rebase makes `last_indexed_commit` unreachable from HEAD, causing `git log hash..HEAD` to return wrong results or fail | Medium | High | Mitigated: before incremental index, verify `last_indexed_commit` is an ancestor of HEAD via `git merge-base --is-ancestor`. If not, fall back to full re-index automatically. |
| T19 | **Schema migration on upgrade** -- new columns or tables in future versions cause errors on existing graph DBs | Low | Medium | Store `schema_version` in `index_state`. On open, check version and run forward migrations (ALTER TABLE ADD COLUMN, CREATE TABLE IF NOT EXISTS). Major version bumps suggest `graph reset`. |
| T20 | **Concurrent same-source sessions** -- two Claude Code instances writing to the same session produces interleaved action sequences | Medium | Medium | Mitigated: `instance_id` field on sessions (defaults to `$PPID`) distinguishes concurrent agents of the same source. Session lookup scoped by `source + instance_id`. |
