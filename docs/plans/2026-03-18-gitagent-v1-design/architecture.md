...
| `commit.go` | `git agent commit` subcommand; parse flags → call `CommitService` → format output |
...
```go
// ProjectConfig is the value object written to .git-agent/project.yml
...
func (s *InitService) Execute(ctx context.Context, input *InitInput) (*InitResult, error) {
    ...
    // 4. Write .git-agent/project.yml (respects Force flag)
...
```bash
#!/bin/sh
# .git-agent/hooks/pre-commit — generated by ga init
...
```bash
#!/bin/sh
# .git-agent/hooks/pre-commit — conventional commit validator
# Built-in hook installed by: git agent init --hook conventional
...
Three-layer resolver: flag → `~/.config/git-agent/config.yml` → `.git-agent/project.yml` → default.

```go
// resolver.go: flag → user config file → default (for scalar values)
type Resolver struct {
    flags      *pflag.FlagSet
    projectCfg *ProjectConfig // loaded from .git-agent/project.yml
    userCfg    *UserConfig    // loaded from ~/.config/git-agent/config.yml
}
...
// user.go: reads ~/.config/git-agent/config.yml (personal, not committed)
...
// project.go: reads .git-agent/project.yml (team config, version-controlled)
type ProjectConfig struct {
    Scopes []string `yaml:"scopes"`
}
...
`model` and `co-author` remain per-commit decisions, but user defaults come from `~/.config/git-agent/config.yml`.
...
`gopkg.in/yaml.v3` is the only addition — used for `.git-agent/project.yml` and `~/.config/git-agent/config.yml` parsing.
