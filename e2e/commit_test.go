...
// TestAutonomousRoot_AutoInitScopeAndGitignore verifies that running bare git-agent in a fresh
// repo with no .gitignore and no .git-agent/config.yml actively discovers and
// initializes both scope and gitignore, and commits the changes.
func TestAutonomousRoot_AutoInitScopeAndGitignore(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	c.Stdout = &stdout
	if err := c.Run(); err != nil {
		t.Fatalf("git-agent root failed: %v\nstderr: %s\nstdout: %s", err, stderr.String(), stdout.String())
	}

	gitignorePath := filepath.Join(dir, ".gitignore")
	if _, err := os.Stat(gitignorePath); os.IsNotExist(err) {
		t.Errorf("expected .gitignore to be auto-generated by autonomous root agent")
	}

	configPath := filepath.Join(dir, ".git-agent", "config.yml")
	configData, err := os.ReadFile(configPath)
	if err != nil {
		t.Errorf("expected .git-agent/config.yml to be auto-generated by autonomous root agent: %v", err)
	} else if !strings.Contains(string(configData), "cli") {
		t.Errorf("expected generated scope 'cli' in config.yml, got:\n%s", string(configData))
	}
}

// TestCommitCmd_SkippedAutoInitOnDisk verifies that running explicit git-agent commit
// does not write .gitignore or .git-agent/config.yml to disk.
func TestCommitCmd_SkippedAutoInitOnDisk(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		bodyBytes, _ := io.ReadAll(r.Body)
		bodyStr := string(bodyBytes)
		w.Header().Set("Content-Type", "application/json")
	...
	gitignorePath := filepath.Join(dir, ".gitignore")
	if _, err := os.Stat(gitignorePath); !os.IsNotExist(err) {
		t.Errorf("expected .gitignore NOT to be created on disk during explicit commit subcommand")
	}

	configPath := filepath.Join(dir, ".git-agent", "config.yml")
	if _, err := os.Stat(configPath); !os.IsNotExist(err) {
		t.Errorf("expected .git-agent/config.yml NOT to be created on disk during explicit commit subcommand")
	}
}
